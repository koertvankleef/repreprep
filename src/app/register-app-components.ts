import './rrr-app.ts'
import './components/rrr-workout-list.ts'
import './components/rrr-workout-editor.ts'
import './components/rrr-exercise-entry.ts'
import './components/rrr-set-entry.ts'
import './components/rrr-exercise-catalogue.ts'
import './components/rrr-exercise-detail.ts'
import './components/rrr-import-export.ts'
import './components/rrr-exercise-history.ts'
import './components/rrr-routine-list.ts'
import './components/rrr-routine-detail.ts'
import './components/rrr-routine-editor.ts'
import './components/rrr-settings.ts'
import './components/rrr-appearance-settings.ts'
import './components/rrr-language-settings.ts'
import './components/rrr-styleguide.ts'
import './components/rrr-workout-logging-flow.ts'
import './components/rrr-workout-logging.ts'

export function registerAppComponents(): void {
  // Side-effect imports above register custom elements once.
}
