...
export class RrrWorkoutLoggingPrototype extends HTMLElement {
  ...
  private updateLiveStageVisuals(): void {
    ...
    if (this.stage === 'transition') {
      ...
      const fill = this.shadowRoot.querySelector<HTMLElement>('.timeline-item--transition[data-state="active"] .bar > span')
      ...
      if (fill) {
        const progressWidth = `${(this.nextExerciseRemainingSeconds / EXERCISE_TRANSITION_SECONDS) * 100}%`
          ...
          ${TIMELINE.map((item, index) => {
            ...
            if (item.kind === 'set') {
              ...
              return `
                <section class="timeline-item timeline-item--set" data-state="${state}">
                  ...
                  <div class="last-time">Last time: ${exercise.previousPerformance}</div>
                  ${isActiveSet
                    ? `
                      <div class="rep-row">
                        <rrr-button type="button" variant="outline" data-action="rep-minus" aria-label="decrease reps">-</rrr-button>
                        <div class="rep-value">${exercise.name === 'Plank' ? `${this.repValue * 5} sec` : `${this.repValue} reps`}</div>
              ...
              return `
                <section class="timeline-item timeline-item--rest" data-state="${state}">
                  ...
                  <div class="rest-header">
                    <div class="rest-title"><rrr-icon name="timer"></rrr-icon>Rest</div>
                  ...
                  <div class="timer">${isActiveRest ? this.formatClock(this.restRemainingSeconds) : this.formatClock(item.durationSeconds)}</div>
                  ${isActiveRest
                    ? `
                      <div class="actions">
                        ${this.stage === 'rest'
            ...
            return `
              <section class="timeline-item timeline-item--transition" data-state="${state}">
                <div>Next: ${nextExercise ? nextExercise.name : 'Workout complete'}</div>
                <div class="countdown">
                  <div class="bar"><span style="width: ${isActive ? progressWidth : '100%'};"></span></div>
                  <div class="count">${isActive && (this.stage === 'transition' || this.stage === 'transition-paused') ? this.nextExerciseRemainingSeconds : item.durationSeconds}</div>
                </div>
                ${isActive && (this.stage === 'transition' || this.stage === 'transition-paused')
                  ? `
                    <div class="actions">
                      ${this.stage === 'transition'
                        ? `<rrr-button type="button" variant="secondary" data-action="stay-here">Stay Here</rrr-button>`
                        : `<rrr-button type="button" variant="secondary" data-action="next">Next</rrr-button>`}
                      <rrr-button type="button" variant="outline" data-action="next-now">Next Now</rrr-button>
                    </div>
                  `
                  : `
                    <div class="hint">${state === 'complete' ? 'Transition completed' : 'Scheduled'}</div>
                  `}
              </section>
            `
          }).join('')}
